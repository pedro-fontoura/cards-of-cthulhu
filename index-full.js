...
A0.Init = function ( onResize, onClick ) {
	...
	console.info( "[App] Pages:", this.lst_pages.map( e => e.id ) );
...
A0.onResize = function ( evt ) {
	// console.info(
		// "[App] onResize:", window.visualViewport.width, "x",  window.visualViewport.height
	// );
...
A0.onClick = function ( js_entity, evt ) {
	// console.log( "[App] onClick >>", js_entity );
...
A0.onContextMenu = function ( evt ) {
	// console.log( "[App] onContextMenu >>", evt.target ); 
	this.ConsumeEvent( evt );
	evt.target.click();
};
...
A0.RegisterServiceWorker = function ( sw_file ) {
	if (
		...
		.then(
			( reg ) => {
				console.log( "[App] RegisterServiceWorker >> Success. Scope: " + reg.scope );
			}
		).catch(
			( error ) => {
				console.error( "[App] RegisterServiceWorker >> Fail: " + error );
			}
		);
	};
};
...
class Card extends FlexBoxEntity {
	...
	constructor ( id, suit, value ) {
		// console.log( "Card >>", id, suit, value );
		super( "Card_" + id, true );
	...
	SetLocation ( entity ) {
		console.assert( entity instanceof FlexBox );
	...
	SetHidden ( status ) {
		// console.log("set hidden:", this.#id, status );
...
class CardGame {
	...
	constructor () {
		...
		console.log( ". tom_cost:", this.#tmo_cost );
	...
	#onResize ( width, height ) {
		...
		console.log( "onResize:", CARD_WIDTH, CARD_HEIGHT );
	...
	#MoveGroupOfCards ( time_factor, nr_cards, fn ) {
		...
		console.log( "MoveGroupOfCards >> nr_cards:", nr_cards );
	...
	#DrawCardsToCardRow ( draw_pile, card_row, nr_cards, to_end = true ) {
		console.log( "DrawCardsToCardRow >>", card_row.GetId(), Math.min( nr_cards, draw_pile.GetNrCards() ) );
	...
	#SendCardsToPile ( lst_cards, target_pile ) {
		console.assert( target_pile instanceof CardPile );
	...
	#Warn ( txt ) {
		if ( this.#continue === null ) {
			...
			console.warn( "#Warn >> continue NOT null");
			console.log( "txt:", txt );
		}
	}
	...
	#onClick ( js_entity, evt_target ) {
		// console.log( "onClick >>", evt_target.id );
		...
		if ( this.#continue !== null ) {
			if ( js_entity === this.#messenger ) {
				this.#continue();
				console.assert( this.#continue === null );
			} else {
				console.warn( "onClick >> CONTINUE not NULL" );
				console.log( js_entity );
				console.log( evt_target );
			}
			return;
		}
		if ( this.#busy ) {
			console.warn( "onClick >> I'm busy" );
			return;
		}
		if ( this.#game_over ) {
		...
		if ( js_entity instanceof Card ) {
			...
			console.log( "onClick >>", evt_target );
		}
	}

	OnClickDie ( die, evt_target ) {
		console.log( "OnClickDie >>", die.GetId() );
	...
	#onClickMenuIcon ( id ) {
		console.log( "onClickMenuIcon >>", id );
	...
	#onClickCard ( card ) {
		console.log( "onClickCard >>", card.ToString() );
		
		this.clicked_card = card; //** DEBUG

		if ( this.#busy ) {
			console.warn( "onClickCard >> I'm busy" );
			return;
		}
	...
	TestAttackCard ( card ) {
		...
		console.info( "TestAttackCard >>", card.ToString() );
		...
		console.log( "-- Attack is OK" );

		if ( this.#attack_cult === NO_CULT ) {
			this.#attack_cult = CULT.GetId();
			CULT.SetUnderAttack( true );
			this.#store.Show();
			console.log( "Attacking Cult:", this.GetCultName( this.#attack_cult ) );
		} 
	...
	PerformCardAttack ( cult, card ) {
		console.log( "PerformCardAttack >>", card.ToString() );
		console.log( ". attack_nr:", this.#attack_nr );
					...
					if ( this.#attack_nr === 1 ) {
						if ( cult.HasTargets() ) {
							...
							return this.ResetAndRollDice().then(
								_ => {
									console.log( "Ready for 2nd attack" );
									this.#store.Show();
								}
							);
						} else {
							console.log( ". No targets for 2nd attack" );
							this.#dice_row.SetAllAsUsed( true );
							this.#busy = false;
						}
					} else {
						console.log( ". No 3rd attack! #attack_nr:", this.#attack_nr );
					}
				}
			}
		);
	...
	GetCultName ( cult_id ) {
		if ( cult_id === NO_CULT ) {
			return console.error( "GetCultName >> No Cult!");
		}
	...
	ValidateAttack ( lst_dice_values, card ) {
		...
		if ( card.IsRoyal() ) {
			...
			console.assert( CARD_VALUE === ROYAL_KING );
		...
		console.assert( card.IsAce() );
		
		return false;
	}

	#onClickCardPile ( card_pile ) {
		console.log( "onClickCardPile >>", card_pile.GetId() );

		if ( this.#busy ) {
			console.warn( "onClickCardPile >> I'm busy" );
			return;
		}
	...
	#Start () {
		console.info ( "--- CardGame >> Start" );

		if ( this.#busy ) {
			console.warn( "Start >> i'm busy" );
			return;
		}
	...
	FindFullCult () {
		...
		console.log( ". summon_cult_id:", summon_cult_id );
	...
	DrawCards ( nr_planets ) {
		console.info( "DrawCards >> Planets:", nr_planets );
	...
	PlaceDrawnCard () {
		...
		console.info( "PlaceDrawnCard >>", CARD.ToString() );
		CARD.SetFaceUp( true );
	...
	PlaceDrawnHorror ( cult, drawn_horror ) {
		console.info( "PlaceDrawnHorror >>", this.GetCultName( cult ), drawn_horror.ToString() );
		console.assert( cult instanceof Cult );
		console.assert( drawn_horror.IsRoyal() );

		console.log( ". nr cultists:", cult.GetNrCultists() );
		console.log( ". needed:", drawn_horror.GetValue() - 10 );

		if ( cult.GetNrCultists() < drawn_horror.GetValue() - 10 ) {
			//** Horror cannot be summoned
			console.log("#0.A - Horror cannot be summoned");
			return this.AddTrophy( drawn_horror );
		}

		if ( ! cult.HasHorror() ) {
			console.log("#0.B - Place horror into empty slot");
			return this.#MoveGroupOfCards(
				ONE_CARD_TIME_FACTOR,
				1,
				_ => {
		...
		console.log( "Cult has a horror:", horror.ToString() );
		console.assert( horror.IsRoyal() );
		console.assert( horror.GetSuit() === drawn_horror.GetSuit() );

		//** Discard the lower value of the two horrors
		if ( horror.GetValue() < drawn_horror.GetValue() ) {
			console.log( "#1 - Swap horror cards");
			return this.#MoveGroupOfCards(
				ONE_CARD_TIME_FACTOR,
				1,
				_ => {
	...
	PlaceDrawnCultist ( cult, card ) {
		console.info( "PlaceDrawnCultist >>", cult.GetId(), card.GetId() );
		console.assert( card.GetValue()  <= 10 );
	...
	PlaceDrawnPlanet ( cult, card ) {
		console.info( "PlaceDrawnPlanet >>", cult.GetId(), card.GetId() );
		console.assert( card.IsAce() );
	...
	AddTrophy ( card ) {
		console.info( "AddTrophy >>", card.ToString(), card.GetLocation() );
	...
	ResetAndRollDice () {
		...
		return this.#dice_row.Roll().then(
			lst_results => {
				console.log( "ResetAndRollDice: Roll is done" );
	...
	ProcessRolledDice ( lst = null ) {
		console.info( "ProcessRolledDice" );

		if ( lst === false ) {
			console.log( ". This was a die reroll" );
		}
		...
		for ( idx = 0; idx < lst.length; ++ idx ) {
			data = lst[ idx ];
			console.log( "| idx:", idx, "die:", data.id, "value:", data.value );
		...
		console.log( "| Nr 1s:", nr_1s, "of", lst.length );
	...
	StartAttackPhase () {
		console.info( "-- StartAttackPhase >> Attack:", 1 + this.#attack_nr );
	...
	SufferInjury ( to_many_1s, nr_1s = 0 ) {
		console.info( "-- SufferInjury >> to_many_1s:", to_many_1s );

		const NR_INJURIRES = 1 + this.#dice_row.NrDeadDice();
		console.log( "SufferInjury >> nr_injuries:", NR_INJURIRES, "/", MAX_INJURIES );
		console.log( ". tmo_cost:", this.#tmo_cost );
			...
			() => {
				if ( NR_INJURIRES === MAX_INJURIES ) {
					...
					if ( to_many_1s && this.#tmo_cost > 0 ) {
						...
						console.log( "COST:", COST, "vs", this.#trophies_pile.GetNrCards() );
	...
	GameOver ( status, reason = null, suit_id = null) {
		console.log( "=== Game over:", status, reason );
	...
	EndAttackAndDraw () {
		console.info( "EndAttackAndDraw >> defeated_targets:", this.#defeated_targets );
		...
		if ( ! this.#defeated_targets ) {
			console.log( ". No defeated_targets" );
	...
	onClickStoreItem ( e_item ) {
		console.info( "onClickStoreItem >>", e_item.id );

		const NR_TROPHIES = this.#trophies_pile.GetNrCards();
		const TEST_ITEM = this.#store.TestItem( e_item, NR_TROPHIES, this.#blood );
		console.log( ". status:", TEST_ITEM );
		
		if ( TEST_ITEM === null ) {
			return console.error( "Unexpected NULL test result for:", e_item );
		}
		...
		if ( isNaN( item_cost ) ) {
			return console.error( "Unexpected item value for:", e_item );
		}

		// console.log( "# Paying", item_cost, " trophies for store item:", item_code );

		this.#store.Hide();

		// this.#blood -= TEST_ITEM.blood_cost;
		// console.assert( this.#blood >= 0 );
	...
	ValidateItemCode ( item_code ) {
		console.info( "ValidateItemCode >>", item_code );

		console.assert( item_code !== STORE_NO_FUNDS );
	...
	UnselectPlanet () {
		console.assert( this.#selected_planet instanceof Card );
		this.#selected_planet.SetSelected( false );
		this.#selected_planet = null;
	}

	TestAttackPlanet ( card ) {
		console.info( "TestAttackPlanet >>", card.ToString() );

		if ( card.IsSelected() ) {
			console.log( ". Unselect selected planet" );
			console.assert( this.#selected_planet === card );
			this.UnselectPlanet();
			return;
		}
		...
		if ( CULT.GetPlanetRound() !== this.#round ) {
			...
			if ( this.#planet_protection !== PLANET_PROTECTION_NONE ) {
				if ( this.#planet_protection === PLANET_PROTECTION_ALL && CULT.HasTargets() ) {
					console.log( ". Planet is protected" );
		...
		if ( ! this.#store.HasAlignmentRequest() ) {
			console.log( ". No alignment request" );
			return this.#Warn( "Request an ALIGNMENT CONTROL from the store." );
		}
		
		if ( this.#selected_planet !== null ) {
			console.log( ". Unselect existent selection:", this.#selected_planet.ToString() );
			this.UnselectPlanet();
		}

		console.log( ". Select planet: ", card.ToString() );
	...
	TestSacrifice ( card ) {
		console.info( "TestSacrifice >>", card.ToString() );

		if ( ! this.#store.HasSacrificeRequest() ) {
			console.log( ". No sacrifice request" );
		...
		console.log( ". trophies:", this.#trophies_pile.GetNrCards() );
		...
		// console.log( "# Paying", COST, "trophies to sacrifice", card.ToString() );
	...
	TestPlanetAction ( action ) {
		console.info( "TestPlanetAction >>", action );
		console.assert( this.#selected_planet !== null );

		console.log( ". planet:", this.#selected_planet.ToString() );
		
		const CULT = this.GetCardCultByLocation( this.#selected_planet );
		console.log( ". CULT:", CULT );
		
		const UPON_APPEARENCE = ( CULT.GetPlanetRound() === this.#round );
		console.log( ". UPON_APPEARENCE:", UPON_APPEARENCE );
		...
		// console.log( "# Paying", COST, "trophies to", action, this.#selected_planet.ToString() );
	...
	Pay ( item_cost, item_code, data = null ) {
		...
		console.log( "PAY", item_cost, item_code, data );
		console.assert( item_cost > 0, item_cost );

		const NR_TROPHIES = data.nr_trophies;
		var txt_cost = "";
		if ( item_cost > NR_TROPHIES ) {
			console.assert( this.#use_blood );
			const BLOOD_COST = data.blood_cost;
			console.assert( this.#blood >= BLOOD_COST );
			this.#blood -= BLOOD_COST;
	...
	ApplyStoreItem ( item_code, data = null ) {
		console.info( "ApplyStoreItem >>", item_code );
		...
		if ( item_code === STORE_SHIFT ) {
			...
			return this.ResetAndRollDice().then(
				() => {
					console.log( "Ready to target another cult." );
				}
			);
		}
...
class CardPile extends FlexBox {
	...
	AddCard ( card, to_top = true ) {
		...
		if ( this.#lst_cards.includes( card) ) {
			return console.error( "CardPile::AddCard >> Already in" );
		}
	...
	GetCardFromTop () {
		if ( this.#lst_cards.length === 0 ) {
			console.warn( "CardPile::GetCardFromTop >> I'm empty:", this.#id );
			return null;
		}
	...
	RemoveCard ( card ) {
		...
		if ( ! this.#lst_cards.includes( card) ) {
			return console.error( "CardPile::RemoveCard >>", this.#id, "No such card >>", card );
		}
	...
	SendCardsToPile ( card_pile ) {
		console.assert( card_pile instanceof CardPile );
...
class CardsRow extends FlexBox {
	...
	AddCard ( card, to_end = true ) {
		...
		if ( this.#lst_cards.length === this.#max_cards ) {
			return console.warn( "CardsRow::AddCard >>", this.#id, ">> I'm full" );
		}
		if ( this.#lst_cards.includes( card) ) {
			return console.warn( "CardsRow::AddCard >>", this.#id, ">> Already in", card );
		}
	...
	RemoveCard ( card ) {
		...
		if ( ! this.#lst_cards.includes( card ) ) {
			return console.error( "CardsRow::RemoveCard >> No such card:", card );
		}
	...
	PeekFirstCard () {
		if ( this.#lst_cards.length === 0 ) {
			console.error( "CardsRow::PeekFirstCard >> I'm empty:", this.#id );
			return null;
		}
	...
	GetFirstCard () {
		if ( this.#lst_cards.length === 0 ) {
			console.error( "CardsRow::GetFirstCard >> I'm empty:", this.#id );
			return null;
		}
...
class Deck {
	...
	DiscardCards ( fn ) {
		if ( this.#lst_cards_out.length > 0 ) {
			console.error( "Deck::DiscardCards >> Some cards are out!" );
			return;
		}

		const lst_cards = this.#lst_cards.filter( fn );

		console.log( "Deck::DiscardCards >> Discarding:", lst_cards.length, "from", this.#lst_cards.length );
		...
		console.log( "Deck::DiscardCards >> Remaining:", this.#lst_cards.length );
	...
	Reset () {
		...
		console.assert( this.#lst_cards.length ===this.#total_cards );
	}
	...
	// 	}
	// 	// console.log( "Deck::GetSequenceSuit >>", sequence_suit );
	// 	if ( sequence_suit === null ) {
	// 		return NaN;
	// 	}
	...
	// 	// console.log( "Deck::GetSequenceValue >>", sum_value );
...
class Menu {
	...
	Toggle () {
		// console.log( "toggle", this.#element.style.right );
...
class Messenger {
	...
	Hide () {
		if ( this.#alert ) {
			console.warn( "Messenger::Hide >> alert is TRUE" );
		}
	...
	ShowMessage ( message ) {
		if ( this.#alert ) {
			console.warn( "Messenger::ShowMessage >> alert" );
	...
	onResize ( width, height, top, left, card_width, card_height ) {
		...
		this.ForEachEntity(
			e => {
				...
				// else {
				// 	console.assert( e instanceof FlexBoxEntity );
...
class Store {
	...
	constructor ( element, fnUpdate, use_blood ) {
		...
		console.log( "STORE >> use blood:", this.#use_blood );
	}
	...
	TestItem ( e_item, funds, blood ) {
		...
		console.log( "Store::TestItem >>", item );
		console.log( ". COST", item.cost );
		console.log( ". funds", funds );
		console.log( ". use_blood", this.#use_blood );
		...
		if ( typeof item.cost === "number") {
			...
			console.log( ". blood cost", BLOOD_COST );
	...
	UseReroll () {
		if ( this.#reroll_available === 0 ) {
			console.error( "Store::UseReroll >> None available" )
			return;
		}
	...
	UseSacrifice () {
		if ( ! this.#requested_sacrifice ) {
			console.error( "Store::UseSacrifice >> Not requested" )
			return;
		}
	...
	UseAlignment () {
		if ( ! this.#requested_alignment ) {
			console.error( "Store::UseAlignment >> Not requested" )
			return;
		}
...
class Cult extends FlexBox {
	...
	onResize ( width, height, top, left, cell_width ) {
		// console.log( "Cult::onResize >>", width, height, top, left, cell_width);
	...
	AddCultist ( card ) {
		console.assert( card instanceof Card );
		console.assert( card.GetValue() <= 10 );
		console.assert( ! this.#cultists.IsFull() );
	...
	AddHorror ( card ) {
		console.assert( card instanceof Card );
		console.assert( card.IsRoyal() );
		console.assert( this.#horror.IsEmpty() );
		return this.#horror.AddCard( card );
	}

	AddPlanet ( card, round ) {
		console.assert( card instanceof Card );
		console.assert( this.#planet.GetNrCards() === 0 );
	...
	RemoveCard ( card ) {
		console.assert( card instanceof Card );
		console.log( "Cult::RemoveCard >>", card.ToString() );

		if ( card.IsAt( this.#cultists) ) {
			console.log( ". card is a Cultist" );
			this.#RemoveCultist( card );
		} else if ( card.IsAt( this.#horror ) ) {
			console.log( ". card is a Horror" );
			this.#RemoveHorror( card );
		} else if ( card.IsAt( this.#planet ) ) {
			console.log( ". card is a Planet" );
			this.#RemovePlanet( card );
		} else {
			console.error( "Cult::RemoveCard >> UNEXPECTED" );
		}
	...
	#RemoveCultist ( card ) {
		console.assert( card instanceof Card );
		console.assert( card.GetValue() <= 10 );
		console.assert( ! this.#cultists.IsEmpty() );
...
class Die extends FlexBoxEntity {
	...
	SetBoxSize () {
		if ( this.#e_body.offsetHeight === 0 ) {
			return console.error( "Die::SetBoxSize >>", this.#id, "Zero offsetHeight" );
		}
	...
	Roll () {
		if ( ! this.CanRoll() ) {
			console.error( "Die::Roll >> Cannot!", this.#id );
	...
	_Roll ( nr_roll, resolve ) {
		...
		if ( nr_roll === 1 || nr_roll < 6 ) {
			...
			console.log( "Die::Roll >> #", this.#id, "=", this.#value );
			setTimeout(
				() => {
	...
	Reroll () {
		if ( ! this.CanReroll() ) {
			console.error( "Die::Reroll >> Cannot!", this.#id );
			return Promise.reject( 0 );
		}
...
class DiceRow extends FlexBox {
	...
	Reset () {
		...
		console.assert( this.#lst_dice.length === this.#start_dice, this.#lst_dice.length );
	...
	AddNewDie () {
		console.log( "AddNewDie >>", this.#die_width, this.#die_height );
	...
	AddDie ( die ) {
		console.assert( die instanceof Die );
	...
	RemoveDie ( dead = true ) {
		if ( this.#lst_dice.length === 0 ) {
			console.error( "DiceRow::RemoveDie >> I'm empty" );
			return -1;
		}

		const die = this.#lst_dice.shift();
		console.assert( this.#lst_spirit_dice.indexOf( die ) === -1 );
	...
	GetSelectionValues () {
		...
		console.log( "DiceRow::GetSelectionValues >>", lst );
	...
	RemoveTempDies () {
		...
		while ( this.#lst_spirit_dice.length > 0) {
			...
			console.assert( die.IsTemp() );
			let d = RemoveArrayElement( this.#lst_dice, die );
			console.assert( d === die, d, die );
			this.#lst_extra_dice.push( die );
			this.RemoveEntity( die );
			die.SetDisabled( true );
		}
	...
	ReviveDie () {
		if ( this.#lst_dead_dice.length === 0 ) {
			console.error( "DiceRow::ReviveDie >> None available" );
			return null;
		}
