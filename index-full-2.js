...
class CardGame {
	...
	OnClickDie ( die, evt_target ) {
		...
		if ( evt_target === die.GetClickTarget() ) {
			...
			if ( die.CanReroll() || this.#store.IsRerollAvailable() ) {
				...
				die.Reroll().then(
					_ => {
						return this.ProcessRolledDice( false ); 
					}
				);
			}
		}
	...
	PlaceDrawnCard () {
		const CARD = this.#draw_row.GetFirstCard(); 
	...
	EndAttackAndDraw () {
		...
		if ( ! this.#defeated_targets ) {
			...
			if ( this.HasTargetsToAttack() ) {
				return this.SufferInjury( false ); 
			}
		...
		if ( item_code === STORE_SACRIFICE ) {
			...
			this.#MoveGroupOfCards(
				...
				() => {
					...
					this.#discard_pile.AddCard( data.card );  
					this.#store.Show();
				}
			);
...
class DiceRow extends FlexBox {
	...
	AddNewDie () {
		...
		die.SetDisabled( false ); 
		die.SetSize( this.#die_width, this.#die_height );

		return this.AddDie( die ); 
	}
...
const ONE_CARD_TIME_FACTOR = 0; 

const PAY_TROPHY_TIME_FACTOR = 0.25; 
const PAY_SACRICIFE_TIME_FACTOR = 0.66; 
const PAY_ALIGNMENT_TIME_FACTOR = 0.25; 
